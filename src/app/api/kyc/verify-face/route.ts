 ...
 * Selfie obligatoire. Recto via : fichier | base64 | brouillon DB (KycDraft).
 */
export async function POST(req: NextRequest) {
  try {
    ...
    const selfieBase64 = String(formData.get("selfieBase64") ?? "");
    ...
    // --- Recto : File → base64 client → brouillon DB ---
    ...
    if (isUploadFile(docFrontRaw)) {
      ...
      const docExtRaw =
        (docFrontRaw.name || "doc.jpg").split(".").pop()?.toLowerCase() || "jpg";
      ...
      await writeFile(
        docFrontPath,
        Buffer.from(await docFrontRaw.arrayBuffer()),
      );
    } else {
      const fromClient = bufferFromBase64(docFrontBase64);
      if (fromClient) {
        if (fromClient.length > MAX_FILE_SIZE) {
          return NextResponse.json(
            { error: "Document trop volumineux (max 5 Mo)" },
            { status: 400 },
          );
        ...
        await writeFile(docFrontPath, fromClient);
      } else {
        const draft = await loadKycDocDraft(userId);
        if (draft && draft.buffer.length > 0) {
          docFrontPath = path.join(userDir, "doc_front.jpg");
          await writeFile(docFrontPath, draft.buffer);
        }
      }
    }
    ...
    if (!docFrontPath) {
      ...
      return NextResponse.json(
        {
          error:
            "Document introuvable. Revenez à l'étape photo de la carte, cliquez sur « Analyser mon document », puis continuez jusqu'au selfie.",
        },
        { status: 400 },
      );
    }
    ...
    if (isUploadFile(selfieRaw)) {
      ...
      const selfieExtRaw =
        (selfieRaw.name || "selfie.jpg").split(".").pop()?.toLowerCase() ||
        "jpg";
      ...
      const buf = bufferFromBase64(selfieBase64);
      if (!buf) {
        return NextResponse.json(
